...
weak-star metrizable on bounded sets.  Choose successively a convergent subsequence
at levels $M=0,1,2,\ldots$, each contained in the preceding one, and take the
diagonal sequence whose $j$th term is the $j$th term of the level-$j$ subsequence.
For every fixed $M$ its tail lies in the level-$M$ subsequence, so it converges on
$\A_M$; the consistency proof above then applies unchanged.
\end{remark}
