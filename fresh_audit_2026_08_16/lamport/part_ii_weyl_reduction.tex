...
\begin{resultbox}
\textbf{Reduction theorem.}  To obtain a full
...
\]
If $D=\sum_jc_jW_M(\xi_j)$ and $E=\sum_kd_kW_M(\zeta_k)$, concatenate the two
finite sums in (2.2).  For $a,b\in\C$ this gives
$\rho_M^{(0)}(aD+bE)=a\rho_M^{(0)}(D)+b\rho_M^{(0)}(E)$; representation
independence just proved makes the equality intrinsic.
