...
Three of the five figures are computed from the manuscript's own formulas rather than
drawn, so they are checkable artifacts: the filter W_gamma of Lemma 9.1, the rate
function Psi_gamma of Lemma 10.5, and the tail T_gamma that feeds it.  Requires NumPy
and SciPy, like the other numerics in this repository.
...
def fig_dag(results):
    ...
    for lab in SPINE:
        ...
        if not r:
            continue
        x, y = cols[lab]
        for dep in set(r["uses"]) | set(r["proof_uses"]):
            if dep in cols:
                dx, dy = cols[dep]
...
def main():
    ...
    # sampled curves for the interactive figures
    data = {"note": "Generated by docs/figures/make_figures.py. Do not edit by hand.",
            "gamma": 1.0,
            "filter": {"t": [round(float(x), 4) for x in ts[::4]],
                       "absW": [float("%.6g" % v) for v in ws[::4]], "L1": float("%.6g" % l1)},
            "psi": {"d": [round(d, 3) for d in np.linspace(0, 40, 161).tolist()],
                    "value": [float("%.6g" % psi(d, 1.0, 1.0, l1, T))
                              for d in np.linspace(0, 40, 161)]}}
