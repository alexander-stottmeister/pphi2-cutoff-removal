...
|---|---|
| 𝒫, 2p′ | interaction polynomial; deg 𝒫 = 2p′, a_{2p′} > 0, p′ ≥ 1 |
| ⟨·,·⟩ | inner product, conjugate-linear in the **first** argument |
| C, μ | time-zero covariance C = (2μ)⁻¹, μ = (−∂ₓ² + m₀²)^{1/2} |
| 𝒞_E, 𝒞_H, 𝒢 | the cutoff classes of [Definition 2.1](definitions.md), in the roles fixed there |
...
| 𝔪 | total-variation bound in the spectral-separation lemma, §9 |
...
bound is 𝔪 — it was M₀ in `lamport_gap_to_main.tex`, colliding with the moment constant
