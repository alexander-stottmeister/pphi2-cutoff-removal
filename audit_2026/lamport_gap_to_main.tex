...
\author{Independent audit of \texttt{glimm\_jaffe/m6/main.tex}}
\date{9 August 2026}
...
The derivation below is independent of Euclidean uniqueness.  It does \emph{not}
prove the manuscript's separate ``no symmetry breaking'' corollary; that corollary
omits a load-bearing phase-decomposition hypothesis from companion M3.
\end{resultbox}
...
\section{A corollary that does not follow}

The manuscript's Corollary 9.3 asserts that the uniform gap is incompatible with
coexistence of two clustering phases interchanged by $\theta$, citing companion M3,
Theorem 4.2.  The cited theorem actually assumes all of the following:
...
The contradiction in M3 uses (B4) to identify the locally obtained invariant limit with
that midpoint.  Without (B4), invariance alone does not imply that identification, and
the displayed long-range-order calculation cannot be made.  GJS 1975--1976 prove
phase-transition and pure-phase results in specified strong-coupling models, but the
audited manuscript neither quotes nor derives a theorem matching all four hypotheses for
its Hamiltonian state space.  Therefore the precise rigorous alternatives are:

\begin{enumerate}
\item add (B1)--(B4) explicitly to Corollary 9.3; or
\item delete the corollary and retain only the already proved fact that the constructed
cutoff-net limit is $\theta$-invariant.
\end{enumerate}

This failure is isolated: Corollary 9.3 is not used in any step of Theorem
\ref{thm:main}.
